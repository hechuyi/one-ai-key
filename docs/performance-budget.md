...
- Management event endpoints default to a 100-record page and cap a single page at 1000 records. CLI failure summaries read bounded in-memory sources only. `one-ai-key failures` defaults to 50 records per source and caps each source at 200 records, so combined routing + response-filter output is at most 400 records; it must not introduce persistent failure storage.
- Management failure-evidence output is a sanitized whitelist. Unsafe local identifiers become null; raw credential ids are projected as short irreversible hashes; upstream bodies, matched text, full URLs, key paths, request payloads, and free-form upstream messages are never evidence fields.
...
- Provider/account failure-domain suppression is in-memory runtime state only. Request planning may consult the compiled account/provider breaker maps, but must not persist breaker transitions or query registry storage on the request path. Provider/account cooling is soft: normal and degraded route candidates are preferred before provider-cooling candidates, and provider-cooling is only a last resort when no better candidate remains. Temporary credential cooldown is lower priority than provider-cooling and can only select an already-loaded credential from the in-memory pool as a last resort; it must not perform storage reads, live probes, or background health checks. 429/rate-limit evidence must remain credential scoped and must not open provider-wide suppression.
