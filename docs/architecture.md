...
- LiteLLM's useful primitive is `client model -> deployment candidates`. A deployment has its own upstream model name, API base, authentication material, route weight, retry/cooldown state, and health. one-ai-key maps this to `ModelRoute -> RouteTarget[] -> Channel/Pool`, while keeping individual credential lifecycle inside the pool selector instead of treating a key as an anonymous string.
- New API and One API use a practical `channel + ability` model: a channel binds an upstream endpoint, key material, model list, groups, model mapping, priority, weight, and retry behavior; an ability is the materialized `(group, model, channel)` selection index. one-ai-key keeps the same materialized-selection idea, but splits the large channel object into typed `Provider`, `Account`, `CredentialSet`, `Pool`, `ModelRoute`, `PolicyProfile`, and `RoutingProfile` resources so future management writes do not entangle secrets, protocol adapters, and route policy.
...
Provider/account failure domains are runtime-only suppression state layered above channel health. A provider-unavailable failure still marks the selected channel cooling down or degraded, but it also opens or degrades the selected account and provider failure domains in memory. When upstream supplied an explicit cooldown such as `Retry-After`, channels sharing that account or provider enter a soft provider-cooling route state. Normal candidates are preferred first, degraded candidates remain a better last resort than provider-cooling candidates, and provider-cooling targets are used only when every otherwise valid target is provider-cooling. Inside a frozen fallback chain, provider-cooling targets are skipped while later targets remain, so a shared-account failure does not immediately retry a sibling channel before an external fallback. Single-target default or named-channel forwarding may still use the provider-cooling target as a last resort instead of failing admission with `no_route_candidate`. Rate-limit and quota evidence stay credential-scoped and do not contribute to provider/account suppression. A successful upstream response closes the selected channel's account and provider domains. This is intentionally smaller than OmniRoute's persisted circuit breaker: no database writes, no background half-open worker, and no request-path storage join.
...
Routing telemetry is a best-effort in-memory ring buffer. `routing.telemetry_buffer_capacity` defaults to 1024, has a hard maximum of 4096, and bounds resident telemetry memory; `/management/runtime` exposes the active capacity as `routing_telemetry_capacity`.
...
Response-filter decisions are captured in a separate best-effort in-memory `response_filter_events` ring. `response_filter.event_window_capacity` defaults to 1024, has a hard maximum of 4096, and `/management/response-filter-events` reports both the active `capacity` and cumulative `dropped_events`.

Management domain events are persisted to JSONL when `management.event_log_path` is configured, but only a bounded recent window is retained in memory for `/management/events`. `management.event_window_capacity` defaults to 1024, has a hard maximum of 4096, and `/management/runtime` exposes it as `management_event_window_capacity`. Management JSONL is audit/display and compatibility replay for management lifecycle state; it is not a failure-evidence source for CLI failures output or retry audit.
...
- Same-target transient retry is a last-resort pre-output guard for eligible Chat Completions and Responses traffic. When `route_target_retry.enabled` is true, no frozen route target remains, the request is non-streaming and replayable, the selection is not named-pool forwarding, no bytes have been sent to the client, the effective deadline can fit another attempt, and the provider failure has no `Retry-After` cooldown, the selected channel may be retried once with directive `retry_same_target`. Local transport failures and upstream 5xx transactions both carry `duplicate_charge_risk=unknown`. Guarded-success 2xx failures, cooldown evidence, `/v1/models`, Embeddings, unknown endpoint families, named-pool requests, streaming requests, non-replayable bodies, and partial-output failures do not use same-target retry.
...
Startup and reload configuration now flow through explicit control-plane
boundaries:

```text
YAML -> ConfigSource -> RegistryDocument
RegistryDocument + staged resources -> RegistryOverlay -> RegistryDocument
RegistryDocument + credential repository input -> ConfigCompiler -> ResolvedConfig
ResolvedConfig -> RuntimeAssembler -> AppState runtime snapshot
```

`src/control_plane.rs` owns the named source, overlay, compiler, assembler, and
diagnostic boundaries. `ConfigSource` reads YAML, expands templates, rejects
unknown top-level fields, applies compatibility defaults, and returns a
`RegistryDocument`. `RegistryOverlay` applies persisted staged registry
resources over a bootstrap document while preserving process-owned fields such
as listen address, client-token bootstrap, management bootstrap, limits,
timeouts, routing defaults, response-filter bootstrap, and model groups.
`ConfigCompiler` takes a `RegistryDocument` plus explicit credential repository
input and returns `ResolvedConfig`; process startup chooses the file or SQLite
credential repository before calling the compiler. `RuntimeAssembler` owns
startup and reload assembly side effects: opening runtime stores, replaying
management events, restoring lifecycle snapshots, replacing compiled channel
and catalog snapshots, resizing event buffers, refreshing management allowlists,
and updating the active registry version. `Diagnostics` consumes compiler
projections for offline `check-config` model visibility instead of duplicating
route or catalog semantics from raw YAML.

`src/config.rs` keeps `AppConfig` as the YAML compatibility schema and semantic resolver implementation. `src/registry.rs` owns the registry document/repository compatibility boundary: `YamlRegistryRepository` delegates YAML loading to `ConfigSource` and returns a `RegistryDocument`. `RegistryDocument` is an input-neutral, compatibility-defaulted resource document; the legacy `RegistryDocument::resolve*` entrypoints remain for tests and compatibility, but production startup and registry reload use `ConfigCompiler` so credential repository selection is explicit at the control-plane boundary.
...
- `GET /management/credential-sets/{id}/credentials/{credential_id}/apply-latest-probe/plan`
...
Set-scoped credential management has a separate operator reference boundary. SQLite-backed credential resources expose `credential_ref` values such as `cr:v1:pos:0`, derived from the credential set's persisted resource position rather than from raw key material, fingerprints, hashes, key prefixes, or suffixes. Set-scoped single-credential endpoints accept either the legacy internal credential id for compatibility or `credential_ref` for operator workflows; the server resolves `credential_ref` to the internal id inside the management boundary. Operator CLI commands must display and accept only `credential_ref` for individual credential workflows. File-bootstrap-only mode has no durable non-secret resource coordinate, so list projections may omit `credential_ref` until a writable credential store is configured.

`GET /management/explain/runtime` is the small operator-facing explanation view for runtime source authority and reload state. It reports the active registry generation, active and staged registry versions, whether runtime reload is required, source summaries for registry, credential, and client-token state, staged-vs-runtime comparison, and the last runtime reload timestamp plus stable error reason code when present. Source summaries are intentionally coarse: `source_kind`, `writable`, `persisted`, and credential lifecycle authority where relevant. The endpoint is readonly and uses the same runtime/store handles already held by `AppState`; it does not parse YAML, read credential files, inspect raw registry rows, probe upstreams, or touch the proxy hot path.

The explain response is also a redaction boundary. It may say that runtime came from bootstrap files, SQLite registry storage, SQLite credential storage, or SQLite client-token storage, but it must not expose local paths, DSNs, URL userinfo, token-like query parameters, raw tokens, token hashes, raw credentials, upstream request/response bodies, response-filter matched text, or operator-provided free-form notes. A reload failure is reported by a stable reason code such as `runtime_reload_failed`, not by serializing arbitrary internal error text into the response.

The `one-ai-key reload status` CLI command is a read-only projection wrapper over runtime reload state. It may summarize `active_registry_generation`, `active_registry_version`, `staged_registry_version`, `runtime_reload_required`, `staged_vs_runtime`, and `last_reload` stable reason-code fields. It must not call `POST /management/runtime/reload`, compute staged-registry semantics locally, or inspect YAML or stores. When staged runtime changes are pending, diagnosis may suggest only read-only reload investigation such as `reload status` or `reload diff`; `reload apply --dry-run` is an explicit operator reload-planning command, not a diagnosis next action.

The `one-ai-key reload diff` CLI command is a read-only wrapper over `GET /management/runtime/reload-diff`. It clamps backend output to the typed redacted diff contract and discards backend-provided mutation suggestions. It never calls reload, writes audit events, updates reload timestamps, creates staged registry state, or inspects YAML/stores in the CLI.

The `one-ai-key reload apply` CLI command is split into an explicit dry-run and an explicit mutation. `reload apply --dry-run` reads `/management/runtime` and `/management/runtime/reload-diff`, reports the current expected staged registry version, and does not POST. `reload apply --yes --expected-staged-registry-version <version>` first re-reads `/management/runtime`; if the staged version changed or the expected version is absent, it fails closed without POSTing. When the precondition matches, it sends exactly one management mutation to `POST /management/runtime/reload?expected_staged_registry_version=<version>`.

`POST /management/runtime/reload` requires `expected_staged_registry_version`. Missing or mismatched preconditions return conflict before loading the staged registry document, writing audit events, updating reload timestamps, or rebuilding runtime state. Event append failures and rebuild failures remain real reload attempts and are recorded in `last_reload` only as stable reason codes. This keeps runtime reload explicit and operator-driven while preserving the proxy hot path: client requests continue to read only compiled in-memory route, credential, scope, and policy state.

Health endpoints intentionally answer different questions:

| Endpoint | Question | HTTP status | Status values | Notes |
| --- | --- | --- | --- | --- |
| `GET /health` | Is the process alive enough to answer? | `200` when the handler runs | Existing liveness body | No route, credential, or resilience guarantee. |
| `GET /ready` | Can the service accept client traffic under the backward-compatible readiness contract? | `200` when the shared readiness projection is `ready`, currently `serving_channels > 0`; `503` otherwise | `ready` or `not_ready` | Includes runtime counts and blocking alert counts, but blocking alerts do not by themselves change the historical readiness status. |
| `GET /management/health/serving` | Can current compiled runtime state serve at least one configured route under the same readiness projection? | `200` when `serving` is true, currently `serving_channels > 0`; `503` otherwise | `ready` or `not_ready` | Adds `blocking_reasons`, `relay_suppression`, `retry_pressure`, `credential_spare_capacity`, and `response_filter_alert_summary` for explanation. |
| `GET /management/health/resilience` | Is the service resilient, degraded, or blocked after management auth succeeds? | Always `200` after auth | `ok`, `degraded`, or `blocked` | `blocked` means no serving channel or a credential-set blocking alert; `degraded` covers operator-input alerts, missing spare credentials, active channel cooldown, or response-filter contamination alerts. |

`serving` and `resilience` health summaries are additive management projections. `relay_suppression` summarizes active all-target selected-channel suppression alerts; `retry_pressure` summarizes bounded recent retry/fallback decisions; `credential_spare_capacity` summarizes credential lifecycle counts and credential sets without spare capacity; `response_filter_alert_summary` summarizes management-only filter contamination alerts. These fields do not mutate routing state, credential lifecycle, channel health, provider/account failure domains, response-filter policy, or retry policy. They explain the same compiled runtime and bounded event windows that the proxy already uses or emits.
...
`/management/model-routes` includes each target channel's runtime health and static endpoint capability metadata so an operator can distinguish route configuration from temporary channel cooldown, selected-channel relay suppression, degradation, manual disablement, configured disablement, and declared endpoint-family support without inspecting routing internals. The same endpoint capability projection is exposed on `/management/channels/{id}` and `/management/routing/preview` candidates. It is compiled configuration metadata only: management reads do not call upstreams, probe credentials, reload runtime, reject client requests, or bridge one OpenAI-compatible endpoint family into another.

`/management/routing/preview` is a read-only explanation endpoint for the same route-planning boundary used by proxy requests. It accepts a client-visible model name and an optional client token id or name, applies the token's model/channel scope, explicit model routes or default-channel fallback, channel health, route strategy, and the configured candidate limit, then returns the selected target and per-target inclusion or exclusion reasons. It distinguishes `channel_cooling_down` from `channel_degraded`: active cooldown, including `balance_scope: channel` selected-channel suppression, is a timed exclusion, while degraded is a last-resort health warning. It also keeps disabled/configured-disabled exclusions distinct from transient cooldown so operators can see that automatic recovery will not restore them. If all scoped candidates are excluded by active cooldown, preview reports the same `no_route_candidate` planning outcome that proxy forwarding would use. Provider/account soft cooling appears as `provider_cooling_down` while better candidates exist and as `provider_cooling_down_last_resort` only when the frozen route plan has no better otherwise-valid target. Temporary credential cooldown appears as `credential_cooling_down` while better candidates exist and as `credential_cooling_down_last_resort` only when every otherwise-valid route target lacks an available non-cooling credential. The `admission_summary` field is generated by the route-planning boundary and serialized by management; CLI commands render that projection rather than recomputing route admission. It does not call upstream providers, write events or telemetry, or call credential selection APIs that mutate the pool's sticky current index. Its credential data is limited to selector generation, credential-set id, and aggregate lifecycle counts.
