...
- Same-target transient retry is a last-resort pre-output guard for single-target routes. When `route_target_retry.enabled` is true, no frozen route target remains, the request is non-streaming and replayable, no bytes have been sent to the client, the effective deadline can fit another attempt, and the provider failure has no `Retry-After` cooldown, the selected channel may be retried once with directive `retry_same_target`. A local transport failure carries `duplicate_charge_risk=none`; an upstream 5xx transaction carries `duplicate_charge_risk=unknown`. Guarded-success 2xx failures, cooldown evidence, streaming requests, non-replayable bodies, and partial-output failures do not use same-target retry.
